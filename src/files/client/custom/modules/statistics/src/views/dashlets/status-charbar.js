define('statistics:views/dashlets/status-charbar',
    ['views/dashlets/abstract/base', 'statistics:date-range-store'],
    function (Dep, store) {

    return Dep.extend({
        template: 'statistics:dashlets/status-charbar',
        ...
        fetchStats: function () {
            ...
            Espo.Ajax.getRequest('Statistics/action/getStatusCharbarStats', params)
        ...
        renderChart: function (data) {
            ...
            let successPalette = ['#4caf50','#66bb6a','#81c784','#a5d6a7'];
            let otherPalette   = ['#4e79a7','#f28e2b','#e15759','#76b7b2','#edc948','#b07aa1','#9c755f','#bab0ac','#54a0ff','#6c5ce7','#fd79a8','#00cec9','#e17055','#fdcb6e','#a29bfe'];
            let successIdx = 0, otherIdx = 0;

            let palette = rows.map(r => r.isSuccess
                ? successPalette[successIdx++ % successPalette.length]
                : otherPalette[otherIdx++   % otherPalette.length]
            );

            let values = rows.map(r => Math.max(r.barValue, 0));
            let sum    = values.reduce((a, b) => a + b, 0) || 1;

            let totalConverted = rows.filter(r => r.isSuccess).reduce((s, r) => s + r.count, 0);
            let convPct = total > 0 ? Math.round((totalConverted / total) * 100) + '%' : '—';
            ...
            let svgSegments = this.buildDonut(
                ...
                (row, color, badge) => {
                    let icon = row.isSuccess ? ' <span style="color:#4caf50;font-size:11px;">✓</span>' : '';
                    legendRows += `<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid #f5f5f5;">
                        ...
                        <span style="font-size:12px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:${row.isSuccess ? '700' : '400'};">
                            ${this.escHtml(row.label)}${icon}
                        </span>
            ...
            let svg = `<svg width="220" height="220" viewBox="0 0 220 220" style="display:block;margin:0 auto;">
                ${svgSegments}
                <text x="110" y="102" text-anchor="middle" style="font-size:22px;font-weight:700;fill:#2d8f2d;">${convPct}</text>
                <text x="110" y="126" text-anchor="middle" style="font-size:11px;fill:#aaa;text-transform:uppercase;letter-spacing:1px;">conv.</text>
            </svg>`;

            this.$el.find('.stat-chart-wrap').html(`
                <div style="display:flex;align-items:center;gap:20px;padding:12px 14px;flex-wrap:wrap;">
                    <div style="flex-shrink:0;">${svg}</div>
                    <div style="flex:1;min-width:160px;max-height:260px;overflow-y:auto;">
                        <div style="font-size:10px;color:#aaa;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px;">Статусы лидов</div>
                        ${legendRows}
                    </div>
                </div>
                <div style="padding:4px 14px 8px;color:#bbb;font-size:11px;text-align:right;">
                    Конвертировано: ${totalConverted} · Всего: ${total}
                </div>
            `);
        },
